...
def source_connection(
    ...
    for j in range(int(gates_num / 2) + 1):
        ...
        src_path = pya.Box(
            x0_0, y0, x0_0 + 300, y0 + tran_w + 180 + path_width + 300 + 180
        )
...
def drain_connection(cell, layout, x0, y0, length, gates_num, difusion_hsp, path_width):
    for j in range(int(gates_num / 2)):
        ...
        via1(
            ...
            via1_layer,
            x0_0 + (300 - 190) / 2,
            y0 + tran_w + path_width + 180 + 2 * 180 + 300 + (300 - 190) / 2,
        )
        via1_array(cell, via1_layer, x0_0 + (300 - 190) / 2, y0, 38, 1)

    src_hpath = pya.Box(
        x0 + (length + difusion_hsp),
        y0 + tran_w + path_width + 180 + 2 * 180 + 300,
        x0
        + (gates_num - 2) * (length)
        ...
        + difusion_hsp
        - 80,
        y0 + tran_w + path_width + 180 + 2 * 180 + 2 * 300,
    )
    cell.shapes(layer).insert(src_hpath)
...
def source_tranarray_interconn(x0, y0, path_length, tran_num_row, i):
    ...
    via1(
        cell, via1_layer, x0 - 3 * difusion_hsp + (300 - 190) / 2, y0 + (300 - 190) / 2
    )
    via1(
        cell,
        via1_layer,
        x0 + path_length + 3 * difusion_hsp + (300 - 190) / 2 - 300,
        y0 + (300 - 190) / 2,
    )

    if i < tran_num_row - 1:
        box_2 = pya.Box(
            x0 - 3 * difusion_hsp + 300, y0, x0 - 3 * difusion_hsp, y0 + 300 + tran_vsp
        )
        cell.shapes(m2_layer).insert(box_2)
        box_3 = pya.Box(
            x0 + path_length + 3 * difusion_hsp - 300,
            y0,
            x0 + path_length + 3 * difusion_hsp,
            y0 + 300 + tran_vsp,
        )
...
def drain_tranarray_interconn(x0, y0, path_length, tran_num_row, i):
    ...
    box_drt = pya.Box(
        x0 + path_length, y0, x0 + path_length + 7 * difusion_hsp, y0 + 300
    )
    cell.shapes(layer).insert(box_drt)
    via1(
        cell, via1_layer, x0 - 7 * difusion_hsp + (300 - 190) / 2, y0 + (300 - 190) / 2
    )
    via1(
        cell,
        via1_layer,
        x0 + path_length + 7 * difusion_hsp + (300 - 190) / 2 - 300,
        y0 + (300 - 190) / 2,
    )
    if i < tran_num_row - 1:
        box_d = pya.Box(
            x0 - 7 * difusion_hsp, y0, x0 - 7 * difusion_hsp + 300, y0 + tran_vsp + 300
        )
        cell.shapes(m2_layer).insert(box_d)
        box_drv = pya.Box(
            x0 + path_length + 7 * difusion_hsp - 300,
            y0,
            x0 + path_length + 7 * difusion_hsp,
            y0 + tran_vsp + 300,
        )
...
for i in range(tran_num_row):
    ...
    source_tranarray_interconn(
        x0,
        y0 + tran_w + path_width + 2 * 180 + i * tran_vsp,
    ...
    drain_tranarray_interconn(
        x0 + length + difusion_hsp,
        y0 + tran_w + path_width + 2 * 180 + i * tran_vsp + 300 + 180,
...
layout.write("../../klayout/pass_transistor_routing_test2.gds")
